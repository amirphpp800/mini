export const onRequestPost = async ({ request, env }) => {
  try {
    ...
    // 30s rate limit per chat_id
    const now = Date.now();
    const lastStr = await env.KV.get(`otp:last:${chat_id}`);
    if (lastStr) {
    ...
    await env.KV.put(`otp:last:${chat_id}`, String(now));
    ...
    return new Response(JSON.stringify({ ok: true, next_retry_sec: 30 }), {
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (err) {
